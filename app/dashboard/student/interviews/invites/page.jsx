...
import {
    ...
    ArrowRight, Sparkles, AlertTriangle, UserCircle
} from "lucide-react";
...
export default function InterviewInvitesPage() {
    ...
    async function fetchInvites() {
        try {
            const res = await fetch("/api/student/interviews/invites", { credentials: "include" });
    ...
    const available = invites.filter(iv => !iv.attempt);
    const attempted = invites.filter(iv => iv.attempt);
        ...
        <div className="max-w-5xl mx-auto">
            <motion.div variants={container} initial="hidden" animate="show">
                <motion.div variants={item} className="mb-8">
                    ...
                    <p className="text-muted-foreground text-sm mt-1">Live interviews manually requested by recruiters specifically for you</p>
                </motion.div>
                ...
                {/* Available Invites */}
                <motion.div variants={item} className="mb-8">
                    <h2 className="text-lg font-semibold text-foreground mb-4">Pending Invites ({available.length})</h2>
                    {available.length === 0 ? (
                        <div className="glass-card rounded-2xl p-12 text-center border-purple-500/10">
                            <Sparkles className="h-12 w-12 text-muted-foreground mx-auto mb-4 opacity-30" />
                            <h3 className="text-lg font-semibold text-foreground mb-2">No Targeted Invites</h3>
                            <p className="text-muted-foreground text-sm">Recruiters will send you private invites directly to your email. Check your public Available Interviews feed for general opportunities.</p>
                        </div>
                    ) : (
                            ...
                            {available.map((iv) => (
                                <motion.div key={iv._id} variants={item}>
                                    <Link href={`/interview/${iv.slug}`}>
                                        <div className="glass-card rounded-xl p-5 hover:scale-[1.01] transition-all cursor-pointer group h-full border border-purple-500/20 shadow-[0_0_15px_rgba(168,85,247,0.1)]">
                                            <div className="flex items-start justify-between mb-3">
                                                <div className="flex items-center gap-3">
                                                    <div className="p-2.5 rounded-lg bg-gradient-to-br from-purple-600 to-pink-600">
                                                        <UserCircle className="h-5 w-5 text-white" />
                                                    </div>
                                                    <div>
                                                        <h3 className="font-semibold text-foreground group-hover:text-purple-400 transition-colors">{iv.jobRole}</h3>
                                            ...
                                            <div className="flex items-center gap-4 text-xs text-muted-foreground mt-3">
                                                ...
                                                <span className="flex items-center gap-1 text-purple-400">
                                                    <Sparkles className="h-3 w-3" /> Live Observation
                                                </span>
                                                <span className="flex items-center gap-1">
                ...
                {attempted.length > 0 && (
                    <motion.div variants={item}>
                        <h2 className="text-lg font-semibold text-foreground mb-4">Completed Invites ({attempted.length})</h2>
                        <div className="grid gap-4 sm:grid-cols-2">
                            {attempted.map((iv) => (
